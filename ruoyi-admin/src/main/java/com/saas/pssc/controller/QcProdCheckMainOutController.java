 ...
 */
@Controller
@RequestMapping("/qc/prodcm/out")
public class QcProdCheckMainOutController extends BaseController
{
    private String prefix = "qc/prodcm/out";

    @Autowired
    private IQcProdCheckMainService qcProdCheckMainService;

    @RequiresPermissions("qc:prodcm:out:view")
    @GetMapping()
     ...
     * 查询成品出货检验记录列表
     */
    @RequiresPermissions("qc:prodcm:out:list")
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(QcProdCheckMain qcProdCheckMain)
    {
        startPage();
        qcProdCheckMain.setOperType("1");//出货
     ...
     * 导出成品出货检验记录列表
     */
    @RequiresPermissions("qc:prodcm:out:export")
    @Log(title = "成品出货检验记录", businessType = BusinessType.EXPORT)
    @PostMapping("/export")
    @ResponseBody
    public AjaxResult export(QcProdCheckMain qcProdCheckMain)
    {
        ...
        return util.exportExcel(list, "成品出货检验记录数据");
    }
     ...
     * 新增保存成品出货检验记录
     */
    @RequiresPermissions("qc:prodcm:out:add")
    @Log(title = "成品出货检验记录", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    @ResponseBody
     ...
     * 修改保存成品出货检验记录
     */
    @RequiresPermissions("qc:prodcm:out:edit")
    @Log(title = "成品出货检验记录", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    @ResponseBody
     ...
     * 删除成品出货检验记录
     */
    @RequiresPermissions("qc:prodcm:out:remove")
    @Log(title = "成品出货检验记录", businessType = BusinessType.DELETE)
    @PostMapping( "/remove")
    @ResponseBody
    ...
    {
        ExcelUtil<QcProdCheckMain> util = new ExcelUtil<QcProdCheckMain>(QcProdCheckMain.class);
        return util.importTemplateExcel("成品出货检验记录列表");
    }
    
    /**
     * 导入数据
     */
    @RequiresPermissions("qc:prodcm:out:import")
    ...
    public String importProdCheckMain(List<QcProdCheckMain> list, Boolean isUpdateSupport)
    {
        ...
        {
            throw new BusinessException("导入成品出货检验记录数据不能为空！");
        }
        int successNum = 0;
        int failureNum = 0;
        ...
        for (QcProdCheckMain qcProdCheckMain : list)
        {
            try
            {
                ...
                logger.info("同名成品出货检验记录条数："+dblist.size());
                if (dblist.size()>0) {
                	flag = true;  // 验证是否存在这个成品出货检验记录
				}
                if (!flag)
                {
                    ...
                    // qcProdCheckMain.setIsValid("1");
                    qcProdCheckMainService.insertQcProdCheckMain(qcProdCheckMain);//插入成品出货检验记录
                    successNum++;
                ...
                else if (isUpdateSupport)
                {
                    ...
                    qcProdCheckMain.setUpdateBy(ShiroUtils.getLoginName());
                	qcProdCheckMainService.updateQcProdCheckMain(qcProdCheckMain);//修改成品出货检验记录
                    successNum++;
